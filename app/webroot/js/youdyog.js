$(document).ready(function(){
		...
		$('#div_partner').ready(function() {
			var what_id = $('#select_what').find(":selected").val();
			loadWhereWhichData(what_id);
		});
		
		$('#select_what').change(function() {
			var what_id = $('#select_what').find(":selected").val();
			//alert(what_id);
			loadWhereWhichData(what_id);
		});
...
function loadWhereWhichData(what_id) {
	$.post('/user/getwherewhich',{id:what_id}, function(data) {
		var newdata = jQuery.parseJSON(data);
		var whichs = newdata['whichs'];
		var wheres = newdata['wheres'];
		if(whichs!=null) {
			var len = whichs.length;
			$('#select_which').empty();
			for(i = 0; i < len; i++) {
				$('#select_which').append('<option value = "'+whichs[i]['id']['$id']+'">'+whichs[i]['name']+'</option>');
			}
		}
		if(wheres!=null) {
			len = wheres.length;
			$('#select_where').empty();
			for(i = 0; i < len; i++) {
				$('#select_where').append('<option value = "'+wheres[i]['id']['$id']+'">'+wheres[i]['name']+'</option>');
			}
		}
	});
}
